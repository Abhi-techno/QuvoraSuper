...
export default function LandingPage() {
  ...
  return (
    <div className="h-svh w-full bg-background flex flex-col overflow-hidden select-none touch-none gpu-accelerated relative">
      ...
      {/* Adaptive Footer Sheet - Refined to be more compact */}
      <footer 
        className="shrink-0 w-full glass-thick rounded-t-[3rem] shadow-2xl pt-6 px-8 text-center flex flex-col items-center z-50 border-t border-white/20"
        style={{ paddingBottom: 'calc(1rem + env(safe-area-inset-bottom))' }}
      >
        <div className="w-12 h-1 bg-foreground/10 rounded-full mb-4 opacity-40" />
        ...
        <div className="w-full max-w-sm flex flex-col gap-3">
          <AuthModal 
            defaultTab="register"
            trigger={
              <Button className="w-full h-14 rounded-[1.8rem] font-black text-sm uppercase tracking-widest bg-primary text-white shadow-xl shadow-primary/20 transition-all border-none active:scale-[0.97] hover:scale-[1.01]">
                Get Started
          ...
          <AuthModal 
            defaultTab="login"
            trigger={
              <button className="flex items-center justify-center gap-2 opacity-60 hover:opacity-100 transition-opacity pb-1 active:scale-95 group">
                <span className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground group-hover:text-foreground">Already a member?</span>
        ...
        <div className="mt-4 flex items-center gap-2 opacity-30 pointer-events-none">
          ...
          <span className="text-[9px] font-black uppercase tracking-[0.4em] text-foreground">Trusted by 10M+ Indians</span>
        </div>
      </footer>
    </div>
  );
